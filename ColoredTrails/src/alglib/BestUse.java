...
public class BestUse
{
	...
	*/
	public BestUse(GameStatus gs, PlayerStatus ps, Scoring s, TileGoal goal_type)
	{
     ...
     * @param g
     */
    public BestUse(GameStatus gs, Goal g) {
       	this.gameStatus = gs;
    	this.goal = g; 
    ...
    public static void updateHumanRequestChip(int amount, String agentName, String color ) {
    	...
    	ColoredTrailsBoardGui.game.getHumanPlayers().get(0).addExchange(new Exchange(agentName,amount,new CtColor(color)));
    }
	...
	public String calcNeededChips(GameStatus game) {
		String output = new String();
		for(Player p : game.getAllPlayers()) {
			output += " Player name : " + p.getPlayerName() + "\n";
			Path improvedPath = improvePath(p, game.getGoals().get(0));
	 ...
	 * @param game
	 */
	public String performeExchanges(GameStatus game) {
		String output = new String();
		output += chooseFirstRequest(game);
		output+="Perform These Exchanges : \n";
		for(Player p : game.getAllPlayers()) {
			for(Exchange exchange : p.getExchanges()) {
	 ...
	 * @return
	 */
	boolean checkIfExchangeIsValid(GameStatus game, Player p,Exchange exchange) {
		return true;
	}
	 ...
	 * @param game
	 */
	public String chooseFirstRequest(GameStatus game) {
		String output = new String();
		for(Player p : game.getAllPlayers()) {
				...
				for(CtColor color : chipsAsked.keySet()) {
					...
					for(Player p_2 : game.getAllPlayers()) {
						...
						if(!p_2.getPlayerName().equals(p.getPlayerName())) {
							for(Exchange exchange_2 : p_2.getExchanges()) {
     ...
     * @return String
     */
	public String calcExchanges(GameStatus game) {
		String output = new String();
					...
					for(Player q : game.getAgentPlayers()) {
						...
						if(!q.getPlayerName().equals(p.getPlayerName())) {
							if (q.getChipsNotUsedInBestPath().contains(color)) {
								...
								p.addExchange(new Exchange(q.getPlayerName(),numChipsAskedFor,color));
     ...
     * @return String
     */
    public String calcBestPaths(GameStatus game) {
    ...
    public void calcBestPath(Player p ) {
    	this.playerStatus = new PlayerStatus(p.getPlayerStatus());

    	// !!! the system is not calculating player scores as player state changes !!!
    	PlayerStatus psstart = new PlayerStatus(this.playerStatus);// copy constructor
    ...
    public BestUse(GameStatus gs, PlayerStatus ps, Goal g) {
    	this.gameStatus = gs;
    	this.goal = g;
	 ...
	 */
	public Path improvePath(Player p, Goal g) {
		ArrayList<PlayerStatus> bestPath = p.getBestPathToGoal();
