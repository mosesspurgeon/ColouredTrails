...
/**
 * This implements the thread that handles the admin client commands
 * This thread responds only to client initiated commands and does not
 * push asynchronous messages directly from server
 * 
...
public class AdminHandler extends Thread {
    ...
    private final static Logger LOGGER = Logger
            .getLogger(AdminHandler.class.getName());
    ...
    public void run() {
        ...
        try {
            ...
            LOGGER.log(Level.INFO, Thread.currentThread().getName()
                    + " - Processing messages of admin client " + clientNum);

            String adminClientMsg = null;
            while ((adminClientMsg = reader.readLine()) != null) {
                LOGGER.log(Level.FINE,
                        Thread.currentThread().getName()
                                + " - Message from client " + clientNum
                                + " received: ");
                LOGGER.log(Level.INFO, Thread.currentThread().getName()
                        + " - Message from client received: " + adminClientMsg);
    ...
    public void handleMessage(Message msg) {
        ...
        try {
            ...
            case "NewAdmin":
                ...
                boolean adminAdded = ServerData.getInstance()
                        .addAdmin(adminIdentity);

                replyToClient = ServerMessages
                        .getAdminConnectionResponse(adminAdded, adminIdentity);
            ...
            case "AvailablePlayerlist":
                List<User> availPlayers = ServerData.getInstance()
                        .getAvailablePlayers();
                replyToClient = ServerMessages.getListPlayersResponse(
                        ServerMessages.LISTTYPE_AVAILABLE, availPlayers);
                sendMessage(replyToClient.toJson());
                break;
            case "AllPlayerlist":
                List<User> allPlayers = ServerData.getInstance()
                        .getAllPlayers();
                replyToClient = ServerMessages.getListPlayersResponse(
                        ServerMessages.LISTTYPE_ALL, allPlayers);
                sendMessage(replyToClient.toJson());
                break;
            case "GameConfiglist":
            ...
            case "GameConfigRefresh":
                ...
                replyToClient = ServerMessages
                        .getRefreshGameConfigsResponse(status);
                sendMessage(replyToClient.toJson());
                break;
            case "Gamelist":
                ...
                break;
            case "CreateGame":
                String gameConfig = (String) requestFromClient
                        .get("gameConfig");
                gameId = ServerData.getInstance().createGame(gameConfig);
                replyToClient = ServerMessages.getCreateGameResponse(gameConfig,
                        gameId != null, gameId);
                sendMessage(replyToClient.toJson());
                break;
            case "ViewGame":
                gameId = requestFromClient.getInteger("game");
                Game requestedGame = ServerData.getInstance()
                        .getGameById(gameId);
                replyToClient = ServerMessages.getViewGameResponse(gameId,
                        requestedGame);
                sendMessage(replyToClient.toJson());
                break;
            case "AddPlayer":
                ...
                replyToClient = ServerMessages.getAddPlayerResponse(gameId,
                        playersAdded);
                sendMessage(replyToClient.toJson());
                break;
            case "RemovePlayer":
                ...
                for (int i = 0; i < playersArray.size(); i++) {
                    String userId = playersArray.getString(i);
                    boolean playerRemoved = ServerData.getInstance()
                            .removeUserFromGame(gameId, userId);
                    if (playerRemoved) {
                        playersRemoved.add(userId);
                    }
                }
                replyToClient = ServerMessages.getRemovePlayerResponse(gameId,
                        playersRemoved);
                sendMessage(replyToClient.toJson());
                break;
            default:
    ...
    public void sendMessage(String msg) {
        ...
        LOGGER.log(Level.INFO, Thread.currentThread().getName()
                + " - Message sent to Admin client:" + clientNum
                + " Admin User:"
                + (this.adminIdentity != null ? adminIdentity : "") + msg);
        try {
            writer.write((msg + "\n"));
            writer.flush();
        } catch (IOException e) {
