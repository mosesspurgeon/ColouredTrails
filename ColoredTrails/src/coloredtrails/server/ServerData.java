...
/**
 * This represents all the data the server holds. It contains info about all the
 * games, players and admins
 * 
...
public class ServerData {
    ...
    private Map<String, User> userMap = Collections
            .synchronizedMap(new HashMap<String, User>());

    private Set<String> adminSet = Collections
            .synchronizedSet(new HashSet<String>());

    private Map<Integer, Game> gamesMap = Collections
            .synchronizedMap(new HashMap<Integer, Game>());
    ...
    public synchronized Integer createGame(String gameConfig) {
        ...
        Game game = null;
        try {
            game = new Game(GameConfigManager.getGameConfig(gameConfig)
                    .getGameConfigId());
    ...
    public boolean addUserToGame(Integer gameId, String userId) {
        ...
        if (networkUser.getCurrentGame() != null
                && networkUser.getCurrentGame() > 0)
     ...
     *//*
        * public String getHostByPin(int pin) { for (PlayerConnection p :
        * players) { if (p.getPin() == pin) { return p.getHostip(); } } return
        * "NO_SUCH_PIN"; }
        */
}
