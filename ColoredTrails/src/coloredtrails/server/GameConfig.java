...
/**
 * This represents the config to create a new game. It is used to load files
...
public class GameConfig implements Serializable, Cloneable {
    ...
    public GameConfig(String gameConfigId) {
        try {
            loadGameConfigFromFile(gameConfigId);// load
                                                 // map,scoring,players,colors
                                                 // and goals
    ...
    private void setDescription() {
        description = String.format(
                "%d x %d board; %d humans %d agents %d goals; Scoring (%d %d %d); colors %d; phases %d",
                boardRows, boardCols, numHumanPlayers, numComputerPlayers,
                numGoals, score.goalweight, score.distweight, score.chipweight,
                boardColors, phases.size());

    }

    public void loadGameConfigFromFile(String gameConfigId) {
        String filePath = ServerData.gamesDirectory + gameConfigId + "/"
                + "game" + gameConfigId + ".txt";
        ...
        try {
            ...
            for (int i = 0; i < numHumanPlayers; i++) {
                int numChips = scanner.nextInt();// read an int for a colored
                                                 // chip
