...
/**
 * This implements the thread that handles the messages from players This thread
 * can both listen messages from client and send messages to the client at the
 * same. This is achieved by started a second thread ClientMessageReader only to
...
public class PlayerHandler extends Thread {
    ...
    private final static Logger LOGGER = Logger
            .getLogger(PlayerHandler.class.getName());
    ...
    public void run() {
        ...
        try {
            ...
            LOGGER.log(Level.INFO, Thread.currentThread().getName()
                    + " - Processing messages of client " + clientNum);

            while (isAlive) {
                Message msg = messageQueue.take();
                LOGGER.log(Level.FINE, Thread.currentThread().getName()
                        + " - Processing Message of client " + clientNum);
                LOGGER.log(Level.INFO, Thread.currentThread().getName()
                        + " - Message of client is " + msg.getContent());
    ...
    public void handleMessage(Message msg) {
        ...
        try {
            requestFromClient = (JsonObject) Jsoner
                    .deserialize(msg.getContent());
            System.out.println("Got request from "
                    + (this.user == null ? "Client" + clientNum
            ...
            switch (type) {
            case "NewUser":
                // (String identity, PlayerHandler managingThread, Socket
                // clientSocket,
                // UserType userType
                User newUser = new User(requestFromClient.getString("identity"),
                        this, this.clientSocket, UserType.valueOf(
                                requestFromClient.getString("userType")));
                boolean playerAdded = ServerData.getInstance()
                        .addPlayer(newUser);

                replyToClient = ServerMessages.getNewIdentityResponse(
                        playerAdded, newUser.getUserType(),
                        newUser.getIdentity());
                ...
                break;
            case "Exchange":
                JsonArray exchangeArray = (JsonArray) requestFromClient
                        .get("exchanges");
                List<Exchange> exchanges = new ArrayList<Exchange>();
                for (int i = 0; i < exchangeArray.size(); i++) {
                    Exchange newExchange = new Exchange(
                            (JsonObject) exchangeArray.get(i));
                    exchanges.add(newExchange);
                }
                playerGame = ServerData.getInstance()
                        .getGameById(this.user.getCurrentGame());
            ...
            case "ExchangeApproval":
                JsonArray exchangeApprovalArray = (JsonArray) requestFromClient
                        .get("exchanges");
                List<Exchange> exchangeApprovals = new ArrayList<Exchange>();
                for (int i = 0; i < exchangeApprovalArray.size(); i++) {
                    Exchange newExchange = new Exchange(
                            (JsonObject) exchangeApprovalArray.get(i));
                    exchangeApprovals.add(newExchange);
                }
                playerGame = ServerData.getInstance()
                        .getGameById(this.user.getCurrentGame());
                if (this.user.getCurrentGame() != null && playerGame != null) {
                    playerGame.addExchangeApprovalsByUser(exchangeApprovals,
                            this.user);
                    playerGame.commsReceived(this.user);
                    playerGame.updatePhase();
                }
            ...
            case "QuitGame":
                ...
                if (gameId != null && this.user.getCurrentGame() != null
                        && gameId.equals(this.user.getCurrentGame())) {
                    ServerData.getInstance().removeUserFromGame(gameId,
                            this.user.getIdentity());
                }
            default:
    ...
    public void close() {
        if (this.user != null) {
            if (this.user.getCurrentGame() != null
                    && this.user.getCurrentGame() > 0) {
                // Game playerGame =
                // ServerData.getInstance().getGameById(this.user.getCurrentGame());
                ServerData.getInstance().removeUserFromGame(
                        this.user.getCurrentGame(), this.user.getIdentity());
