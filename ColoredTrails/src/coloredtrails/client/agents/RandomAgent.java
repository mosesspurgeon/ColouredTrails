...
/**
 * This implements agent with random behaviour; picking exchanges and approvals
 * at random
 * 
...
public class RandomAgent extends AgentClient {
    ...
    public RandomAgent(String serverHostname, int serverPort,
            String clientName) {
    ...
    public List<Exchange> selectExchanges() {
        ...
        if (!me.isReachedGoal())// If the player has not reached the goal
        {
            ...
            do {
                int randomPlayerIndex = random
                        .nextInt(this.getGamePlayed().getNumAllPlayers());
                // could be changed to Agent if exchange needs to happen only
                // with agents
                randomPlayer = this.getGamePlayed().getAllPlayers()
                        .get(randomPlayerIndex);
            } while (this.getGameBoardPlayerName()
                    .equals(randomPlayer.getPlayerName()));

            // pick a random color from player's not used chipset
            List<CtColor> notUsedChips = randomPlayer
                    .getChipsNotUsedInBestPath().getChips();
            if (notUsedChips.size() > 0) {
                ...
                exchanges.add(new Exchange(randomPlayer.getPlayerName(),
                        numChipsAskedFor, chipColor));
            }
        }
        return exchanges;
    }
