...
public class Phases implements ActionListener, Serializable, Cloneable, JsonSerializable {
	/**
	...
	public JsonObject toJsonObject() {
		...
		phases.put("phaseSequence", phaseSequence == null ? null : Util.toJsonList(phaseSequence));
		phases.put("previous", (previous == null) ? null : previous.toJsonObject());
		phases.put("current", (current == null) ? null : current.toJsonObject());
		phases.put("phasesElapsed", phasesElapsed);
		phases.put("isLoop", isLoop);
		phases.put("currentelapsed", currentelapsed);
		phases.put("currentleft", currentleft);
		phases.put("time", time);
		return phases;
	}

	public Phases(JsonObject phases) {
		JsonArray array = (JsonArray) phases.get("phaseSequence");
		if (array != null) {
			this.phaseSequence = new ArrayList<GamePhase>();
			for (int i = 0; i < array.size(); i++) {
				phaseSequence.add(new GamePhase((JsonObject) array.get(i)));
			}
		}
		this.previous = phases.get("previous") == null ? null
				: getPhaseFromSequence(new GamePhase((JsonObject) phases.get("previous")));
		this.current = phases.get("current") == null ? null
				: getPhaseFromSequence(new GamePhase((JsonObject) phases.get("current")));
		this.phasesElapsed = phases.getInteger("phasesElapsed");
		this.currentelapsed = phases.getInteger("currentelapsed");
		this.currentleft = phases.getInteger("currentleft");
		this.isLoop = phases.getBoolean("isLoop");
		this.time = phases.getLong("time");

	}

	public GamePhase getPhaseFromSequence(GamePhase search) {
		for (GamePhase gp : phaseSequence) {
			if (gp.getName() != null && gp.getName().contains(search.getName()))
				return gp;
		}
		return null;
	}
	...
	public Phases(Phases p) {
		 ...
		 * this();
		 * 
		 * phasesElapsed = p.phasesElapsed; isLoop = p.isLoop;
		 * 
		 * for( GamePhase h : p.phaseSequence ) { GamePhase g = new GamePhase( h ); if(
		 * p.current == h ) current = g; else if( p.previous == h ) previous = g;
		 * phaseSequence.add( g ); }
		 * 
		 * this.currentelapsed = p.currentelapsed; this.currentleft = p.currentleft;
		 * 
		 * this.time = p.time;
		 */
	}
	...
	/**
	 * Returns the game phase with the specified name or null if specified name not
	 * found
	 *
	 * @param phaseName
	 ...
	 * Gets the next Phase
	 *
	 * @return the next Phase in the list of Phases. If the current Phase is null
	 *         then the first Phase in the list is returned. If the Phase is the
	 *         last Phase and there are no loops then null is returned, otherwise
	 *         the first Phase in the list is returned.
	 */
	...
	/**
	 * Returns the name of the phase that succeeds the specified phase in the Phases
	 * sequence
	 *
	 * @param gp
	 *            phase whose successor we want
	...
	/**
	 * Gets the name of the previous phase that actually executed (NOTE that the
	 * phase that actually preceeded the current phase may have been something other
	 * than what preceeds the current phase in the Phases sequence; this is because
	 * we can manually jump arbitrarily between phases)
	 *
	...
	/**
	 * Gets the name of the phase that appears before the specified phase in the
	 * Phases sequence
	 *
	 * @param gp
	 *            GamePhase to be referenced
	 * @return Name of the previous phase
	 */
	 ...
	 *
	 * @param refPhaseName
	 *            Name of the phase to be referenced
	 * @return Name of the previous phase
	 */
	...
	/**
	 * Ends the current game phase and advances the game to the specified game phase
	 *
	 * @param phaseName
	 *            the name of the phase.
	 */
	...
	/**
	 * Ends the current game phase and advances the game to the next phase in the
	 * Phases sequence (may be invoked manually in configuration class or
	 * automatically by a phase time out)
	 */
	...
	/**
	 * Begins the phase; eventually invokes the beginPhase() in game configuration
	 * class
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((parentGame.getGameStatus() == Status.INPROGRESS) && (getCurrentPhase() != null)
				&& (!getCurrentPhase().isIndefinite())) {
			this.currentleft = getCurrentSecsLeft() - 1;
			// if game phase is not indefinite, and decremented time is <= 0 ...
			if (getCurrentSecsLeft() <= 0) {
				GamePhase nextPhase = this.getNextPhase(this.getCurrentPhase());
				parentGame.advancePhase(nextPhase);
			}
		}
	}
	 ...
	 *
	 * @param phaseName
	 *            Name of the phase whose duration we want
