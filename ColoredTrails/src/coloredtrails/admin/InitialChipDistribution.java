...
/**
 * Creates the right panel on the View Game GUI for an admin It displays 4 items
 * - the chip distribution of game, - Game Status and Phase (alongwith time if
 * phase is definite) - History of all exchange moves and approvals - Summary of
 * the game status
 * 
 *
 * @author Mor Vered -based from coloredtrails.ui.InitialChipDistribution
...
public class InitialChipDistribution extends JPanel {
    ...
    // Timer to display number of secs when a game is in progress
    ...
    public InitialChipDistribution() {
        ...
        for (int index = 0; index < numAllPlayers; index++) {
            int colorIndex;
            String labelString;
            String playerName = AdminGameBoard.game.getAllPlayers().get(index)
                    .getPlayerName();
            ...
            initialChipDistribution.get(index).setBounds(12,
                    firstInitChipRowPosition + spaceBetweenInitChipRows * index,
                    249, 15);
            ...
            for (int i = 0; i < numColors; i++) {
                // The order is : Blue,Red, White, Yellow, Green, Orange, Gray,
                // Brown, Purple,
                // Light blue
                numChipsOfEachColor.add(new JTextField());
                numChipsOfEachColor.get(i)
                        .setBounds(264 + spaceBetweenInitChipColumns * i,
                                firstInitChipRowPosition
                                        + index * spaceBetweenInitChipRows,
                                19, 15);
                numChipsOfEachColor.get(i)
            ...
            colorsChipText.get(colorIndex).setBounds(279,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            ...
            colorsChipText.add(new JTextField());
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 2,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            colorsChipText.get(colorIndex).setBorder(null);
            colorsChipText.get(colorIndex)
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 3,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 4,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 5,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 6,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 7,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 8,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
            ...
            colorIndex = colorsChipText.size() - 1;
            colorsChipText.get(colorIndex).setBounds(
                    279 + spaceBetweenInitChipColumns * 9,
                    firstInitChipRowPosition + index * spaceBetweenInitChipRows,
                    19, 15);
            colorsChipText.get(colorIndex).setEditable(false);
            colorsChipText.get(colorIndex).setColumns(10);
     ...
     * 
     * if(nextPhase!=null) {
     * AdminGameBoard.game.getPhases().setCurrent(nextPhase); phaseName =
     * AdminGameBoard.game.getPhases().getCurrent().getName(); String text =
     * phaseName + " Phase"; lblCommAndTime.setText(text); } else//startOver {
     * GamePhase newPhase =
     * AdminGameBoard.game.getPhases().getPhaseSequence().get(0);
     ...
     * AdminGameBoard.calcNeededChips(); this.btnNext.setEnabled(false); }
     * //Exchange phase else if(phaseName.equals("Exchange")) {
     * AdminGameBoard.exchangeChips(); initChipExchangeValues();
     * this.btnProposeExchange.setEnabled(false); this.btnNext.setEnabled(true);
     * } //Move phase else if(phaseName.equals("Movement")) {
     * this.btnProposeExchange.setEnabled(false); AdminGameBoard.movePlayers();
        ...
        phaseDurationUpdateTimer = new Timer(1000, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (AdminGameBoard.game.getGameStatus() == Status.INPROGRESS
                        && !AdminGameBoard.game.getPhases().getCurrent()
                                .isIndefinite()) {
                    updatePhaseDetails();
                    Phases gamePhases = AdminGameBoard.game.getPhases();
                    gamePhases.setCurrentleft(
                            gamePhases.getCurrentSecsLeft() - 1);
                }
            }
        });
    ...
    void stopTimer() {
        if (phaseDurationUpdateTimer != null
                && phaseDurationUpdateTimer.isRunning())
    ...
    void initChipDistribution() {
        for (int i = 0; i < numAllPlayers; i++) {
            ArrayList<JTextField> playerColors = (ArrayList<JTextField>) this.numColorsPerAgent
                    .get(i);
    ...
    void setPlayerChips(int playerIndex, ArrayList<CtColor> chips) {
        ...
        // The order is : Blue,Red, White, Yellow, Green, Orange, Gray, Brown,
        // Purple,
        // Light blue
        for (CtColor color : chips) {
            int colorNum = color.getColorNum();
            int chipNumber = Integer
                    .parseInt(playerColors.get(colorNum - 1).getText());
            chipNumber++;
            playerColors.get(colorNum - 1)
                    .setText(Integer.toString(chipNumber));
        }
    }
    ...
    public void updatePhaseDetails() {
        ...
        if (currGameStatus == Status.INPROGRESS) {
            ...
            String phaseName = AdminGameBoard.game.getPhases().getCurrent()
                    .getName();
            text += "  - " + phaseName.toUpperCase() + " Phase";
            if (!AdminGameBoard.game.getPhases().getCurrent().isIndefinite()) {
                int timeLeft = AdminGameBoard.game.getPhases()
                        .getCurrentSecsLeft();
    ...
    public String getExchangeInformation() {
        ...
        for (Map.Entry<Integer, Map<String, String>> entry : exchangeHistory
                .entrySet()) {
            int round = entry.getKey();
            exchangeInfo += "Round " + round + ":\n";
            Map<String, String> playerExchanges = entry.getValue();

            for (Map.Entry<String, String> playerExchange : playerExchanges
                    .entrySet()) {
    ...
    public Map<Integer, Map<String, String>> sortExchangesByRounds() {
        ...
        for (Player p : AdminGameBoard.game.getAllPlayers()) {
            ...
            for (Exchange e : p.getExchangeInitiatedArchive()) {
                ...
                String updatedExchange = playerExchangesInRound.concat(
                        "                    " + exchangeDescription(e) + "\n");
    ...
    public String exchangeDescription(Exchange e) {
        ...
        for (CtColor color : e.getChipsAsked().keySet())
            output += e.getChipsAsked().get(color) + " chips of "
                    + color.getColorName() + "; ";
        output += ":         Offered -- ";
        for (CtColor color : e.getChipsOffered().keySet())
            output += e.getChipsOffered().get(color) + " chips of "
                    + color.getColorName() + "; ";
        output += ":         "
                + (e.isOwnerApproved() ? "Approved" : "Rejected");
        return output;
    }
    ...
    public String getGameStatusDetails() {
        ...
        for (Player p : game.getAllPlayers()) {
            ...
            String networkUser = p.getNetworkUser() == null ? "Unloaded"
                    : p.getNetworkUser().getIdentity();
            String playerType = p.getNetworkUser() == null ? "-"
                    : p.getNetworkUser().getUserType().name();
            Double score = (game.getGameStatus() == Status.CREATED) ? 0
                    : p.getNetworkUser() == null ? 0 : scores.get(player++);
            String goalReached = p.getNetworkUser() == null ? "-"
                    : (game.hasPlayerReachedGoalTile(p) ? "yes" : "no");
            String leftGame = p.isLeftGame() ? "yes" : "";

            scoreSum += score;

            table += String.format(
                    "<tr><td>%s</td><td>%s</td><td align='center'>%s</td><td align='center'>%f</td><td align='center'>%s</td><td align='center'>%s</td></tr>",
                    boardPlayer, networkUser, playerType, score, goalReached,
                    leftGame);
