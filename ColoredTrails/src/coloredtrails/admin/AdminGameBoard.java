...
public class AdminGameBoard implements Runnable {
	...
	private final JLabel message = new JLabel("New Colored Trails game is about to begin !");
	...
	public AdminGameBoard(AdminClient adminClient, Game game) {
		...
		AdminGameBoard.ctBoardSquares = new JButton[AdminGameBoard.rows][AdminGameBoard.columns];
	...
	public static void updatePlayerLocationsOnBoard() {
		...
		for (Player player : players) {
			...
			setMultipleImages(player.getCurrentTile().getCoordinates().getxCoord(),
					player.getCurrentTile().getCoordinates().getyCoord(), player, currentTile.getPlayers());
			// setImage(player.getCurrentTile().getCoordinates().getxCoord(),
	...
	public static void updateGoalLocationsOnBoard() {
		...
		for (Goal g : goals) {
			...
			setImage(g.getCurrentTile().getCoordinates().getxCoord(), g.getCurrentTile().getCoordinates().getyCoord(),
					g.getGoalIcon());
		}
	...
	/*
	 * @Deprecated public static void exchangeChips() {
	 * icd.setExchangeAndPathInfo(bestUse.performeExchanges(game));
	 * //performExchanges(game) performs the actual exchanges and returns a string
	 ...
	 * bestUse.calcExchangesServer(game); // adds the exchange for all the agents in
	 * communication phase }
	 */

	/*
	 * @Deprecated public static void calcNeededChips() { String outputOne =
	 * bestUse.calcBestPaths(game); //sets the best path in the player object String
	 * outputTwo = bestUse.calcNeededChips(game); //sets the needed chips String
	 * outputThree = bestUse.calcExchanges(game); // adds the exchange for all the
	 * agents in communication phase icd.setExchangeAndPathInfo(outputOne +
	 * outputTwo + outputThree); }
	 */

	/*
	 * @Deprecated public static void movePlayers() { game.moveOneStep();
	 * //game.moveAllSteps(); update(); }
	 */
	// MOR
	public final void initializeGui() {
		...
		for (int ii = 0; ii < AdminGameBoard.rows; ii++) {
			ctBoard.add(new JLabel(Integer.toString(ii + 1), SwingConstants.CENTER));
		...
		for (int ii = 0; ii < AdminGameBoard.rows; ii++) {
			for (int jj = 0; jj < AdminGameBoard.columns; jj++) {
				switch (jj) {
				case 0:
					ctBoard.add(new JLabel("" + (ii + 1), SwingConstants.CENTER));
				default:
		...
		mainFrame.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent arg0) {
				synchronized (lock) {
					...
					lock.notify();
					if (gameStatusRefresher != null && gameStatusRefresher.isRunning())
	...
	public static BufferedImage getCombinedImage(BufferedImage i1, BufferedImage i2) {
		...
		BufferedImage bi = new BufferedImage(i1.getHeight(), i1.getWidth(), BufferedImage.TYPE_INT_ARGB);
	...
	public static void setImage(int i, int j, String playerImage) {
		...
		try {
			...
			// ctBoardSquares[i][j].setIcon(new
			// ImageIcon(ImageIO.read(getClass().getResource(resource))));
			ctBoardSquares[i][j].setIcon(new ImageIcon(ImageIO.read(AdminGameBoard.class.getResource(resource))));
		} catch (IOException e) {
	...
	public static void setMultipleImages(int i, int j, Player newPlayer, ArrayList<Player> players) {
		...
		{
			try {
				BufferedImage bi1 = ImageIO.read(AdminGameBoard.class.getResource(resource));
				for (Player p : players) {
					// Check for new players
					if (!p.getPlayerName().equals(newPlayer.getPlayerName())) {
						playerImage = p.getPlayerIcon();
						resource = "../resources/" + playerImage + ".png";
						BufferedImage bi2 = ImageIO.read(AdminGameBoard.class.getResource(resource));
	...
	public void updateGuiBoard() {
		...
		for (Player player : players) {
			AdminGameBoard.setImage(player.getCurrentTile().getCoordinates().getxCoord(),
					player.getCurrentTile().getCoordinates().getyCoord(), player.getPlayerIcon());
		}
		...
		ArrayList<Goal> goals = game.getGoals();
		for (Goal g : goals) {
			AdminGameBoard.setImage(g.getCurrentTile().getCoordinates().getxCoord(),
					g.getCurrentTile().getCoordinates().getyCoord(), g.getGoalIcon());
		}

	}

	void startGuiRefreshTimer() {
		gameStatusRefresher = new Timer(REFRESH_INTERVAL_SECS * 1000, new ActionListener() {
			/*
			 * ArrayList<GamePhase> gamePhases = (ArrayList<GamePhase>)
			 * phases.getPhaseSequence();
			 * 
			 * int numOfPhases = gamePhases.size(); int phaseIndex = 0; int start=0;
			 * GamePhase currentPhase = gamePhases.get(phaseIndex); int phaseTimeLeft =
			 * currentPhase.getDuration(); String phaseName = currentPhase.getName();
			 */
			public void actionPerformed(ActionEvent e) {
				refreshGame();
			}
