...
public class AdminClient extends Thread {
	...
	public void run() {
		...
		if (connected) {
			LOGGER.log(Level.INFO, "Admin Connection to the server has been approved");
			consoleIn = new BufferedReader(new InputStreamReader(System.in));
			String command;
			System.out.print("Admin> ");
			try {
				loop: while ((command = consoleIn.readLine()) != null && !command.equals("exit")) {
			...
			LOGGER.log(Level.INFO, "Admin Connection to the server was not approved");
		}
	...
	private void refreshGameConfigs() {
		JsonObject allGameConfigs = syncRequestReply(AdminClientMessages.getRefreshGameConfigsRequest());
		boolean success = allGameConfigs.getBoolean("status");
		System.out.println("  refresh was " + (success ? "successful" : "unsuccessful"));
	...
	public boolean connectToServer() {
		...
		try {
			...
			approved = syncRequestReply(AdminClientMessages.connectRequest(name)).getBoolean("approved");
	...
	public void createGame(String gameConfig) {
		JsonObject createGame = syncRequestReply(AdminClientMessages.getCreateGameRequest(gameConfig));
		boolean success = createGame.getBoolean("status");
		System.out.println("  Game creation was "
				+ (success ? "successful. Game created: " + createGame.getInteger("game") : "unsuccessful"));
	}

	private void addPlayer(String addplayer) {
		String[] args = addplayer.trim().split("\\s+");
		if (args.length < 2) {
			System.out.println("  Player addition requires atleast 2 arguments - [gameid] [playerid]");
		} else {
			List<String> playerIds = new ArrayList<String>();
			for (int i = 1; i < args.length; i++) {
				playerIds.add(args[i]);
			}
			JsonObject addPlayerResponse = syncRequestReply(
					AdminClientMessages.getAddPlayerRequest(Integer.parseInt(args[0]), playerIds));
	...
	private void removePlayer(String removePlayer) {
		...
		if (args.length < 2) {
			System.out.println("  Player removal requires atleast 2 arguments - [gameid] [playerid]");
		} else {
			List<String> playerIds = new ArrayList<String>();
			for (int i = 1; i < args.length; i++) {
				playerIds.add(args[i]);
			...
			JsonObject removePlayerResponse = syncRequestReply(
					AdminClientMessages.getRemovePlayerRequest(Integer.parseInt(args[0]), playerIds));
	...
	private void send(String message) throws IOException {
		LOGGER.log(Level.FINE, Thread.currentThread().getName() + "Sending: " + message);
		out.write((message + "\n").getBytes("UTF-8"));
		out.flush();
